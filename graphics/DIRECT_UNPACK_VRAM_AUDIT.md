...
| Seasonal `func_08077EC0` BG group | non-winter `08755848`, winter `0875822C`, palettes `08757AE0` / `0875A358`, and paired `30×13` maps | The complete non-winter group is byte-identical in JP/US/EU/DE. DE physically reorders it (`4DCD64` and `4DEFFC` for stream/palette), so assembly selects exact regional build files. | The routine chooses winter only when `func_0800E324` returns season 3; both streams decode to 0x8000-byte 4bpp tile sets. The non-winter `220` group is managed: a 256×256 indexed tile source, six-bank BGR555 palette source and two native maps rebuild each original range exactly. `encode_huff8_lz2` preserves unchanged retail bytes and produces strictly decoded edits inside the `0x2298` slot, with original ladder `25810111214`. `reference/bg_30.png` and `reference/bg_29.png` are code-backed separate BG-layer views, not map-editing inputs. The winter `230` stream remains unmanaged because compatible H8/LZ3 output is `0x2138`, 12 bytes over the `0x212C` slot. |
...
| `asm/intro_scene.s` direct consumers | `0874E648`, `0874EB60`, `0874A9C0` | `0874A9C0` and its immediately following `0x200` palette are byte-identical in all four retail ROMs; `0874E648`/`0874EB60` differ from JP. The DE physical group begins at `0x4D1E2C`, not the general `+0x2080` mapping used by earlier data. | Unclassified by this audit. Existing Intro Scene pipelines cover other proven object/background streams separately; the assembly now uses the verified DE-specific ranges for this relocated group. |
