...
| Overseas `func_08077EC0` seasonal BG group | non-winter `08755848`, winter `0875822C`, palettes `08757AE0` / `0875A358`, and paired `30×13` maps | US/EU/DE payloads are byte-identical. DE physically reorders the group (`4DCD64` / `4DF748`, etc.), so it must use explicit regional boundaries. JP retains a separately compiled state path. | Classified but not yet managed. The routine chooses the winter group only when `func_0800E324` returns season 3; both streams decode to 0x8000-byte 4bpp tile sets. Non-winter uses format `220`; winter uses `230`. The six-bank palettes and native maps are independently bounded. |
