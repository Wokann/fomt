...
| `func_080B55D0` area | `086FB004`, `0872C5D0`, `0872C73C`, `0872C84C` | All labelled ranges are identical | Main `086FB004` uses native format `230`, for which no verified encoder exists. The three later streams have decodable `020`/`030` formats but are not isolated from the larger scene yet. |
| `func_080BD064` area | `0873BE24`, `0873BF48`, `0873BFF0` | All labelled ranges are identical | The two 0x800-byte streams are tilemap candidates. The final 0x7BE2-byte payload has no confirmed native image layout. |
| `func_080C160C` area | `0873F3C8`, `0873F5D4`, `0873F6AC` | All labelled ranges are identical | Two 0x800-byte tilemap candidates and one 0xE80-byte decoded payload; layout/palette relationship still needs review. |
| `asm/code_0803EE94.s` isolated consumers | `08738D1C`, `0872F21C`, `08743058`, `0852AA6C`, `0874E648`, `0874ECCC`, `0875822C` | Most full labelled ranges are shared; `0874E648` and `0874ECCC` are not byte-identical with JP | Unclassified. Two ranges have no valid `Unpack` header at their assembly boundary; other ranges use either unproven format `230` or need consumer/layout analysis. |
