...
| Small companion indexed archive | `IndexedResourceArchive` parses its six counted descriptor sections and final entry table; its payload is byte-identical across all four regions. | Exact boundaries and regional locations are recorded in `INDEXED_RESOURCE_ARCHIVES.md`. Its descriptor semantics still require consumer-by-consumer analysis and it is not falsely exported as a graphics sheet. |
