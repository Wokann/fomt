...
| Overseas `func_08077810` BG tilemap | `graphics/ui/scene_08077810/shared/layer_0.tilemap` | one Huffman-4/LZ3 32-by-32 BG tilemap included directly by `gUnk_0874ECCC` in US/EU/DE | Yes for JP/US/EU/DE: JP has no corresponding consumer and remains direct ROM data |
...
| Direct `gUnk_08698E14` / `gUnk_0869A0A4` field VRAM tile groups | `graphics/ui/raw_vram_tiles/{08698e14,0869a0a4}/shared/tiles.4bpp` | two fixed raw 143-tile 4bpp ranges selected by the field renderer; each is DMA-patched after linking at the original regional offset; palette and layout remain unproven | Yes |
...
| Direct UI DMA tile groups | `graphics/ui/raw_vram_tiles/{08750c4c,08750c8c,087510ac,0875166c}/shared/tiles.4bpp` | four fixed raw 4bpp character-tile records, DMA-patched after linking at their original regional offsets; no palette or UI layout is claimed | Yes |
...
| Seasonal overseas background group | `func_08077EC0` selects paired non-winter/winter 0x8000-byte tile streams, six-bank palettes and two 30x13 tilemaps. | All US/EU/DE payloads are identical but DE is physically reordered; format `220` requires its own fixed-slot encoder before the group can become editable. |
