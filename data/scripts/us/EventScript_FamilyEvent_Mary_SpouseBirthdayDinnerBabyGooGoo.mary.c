...
mary_text_table
{
    const char gText_FamilyEvent_Mary_SpouseBirthdayDinnerBabyGooGoo[] =
        "Goo-goo.{Press}";
};

void EventScript_FamilyEvent_Mary_SpouseBirthdayDinnerBabyGooGoo(void)
{
    if (VarGet(VAR_MARY_CHILDBIRTH_EVENT_STATE) == EVENT_LIFECYCLE_COMPLETED)
    {
        if (VarGet(VAR_CHILD_AGE_DAYS) >= CHILD_AGE_DAYS_FAMILY_SCENE_AND_INJURY_EVENT_START)
        {
            ...
            TalkMessage(gText_FamilyEvent_Mary_SpouseBirthdayDinnerBabyGooGoo);
            TalkClose();
