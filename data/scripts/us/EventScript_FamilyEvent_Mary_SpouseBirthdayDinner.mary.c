...
void EventScript_FamilyEvent_Mary_SpouseBirthdayDinner(void)
{
    ...
    SetEntityPosition(ENTITY_MARY, X(327), Y(114), FACING_DOWN);
    ...
    SetEntityEventScript(ENTITY_MARY, EventScript_FamilyEvent_Mary_SpouseBirthdayDinnerDialogueChoice);
    if (VarGet(VAR_MARY_CHILDBIRTH_EVENT_STATE) == EVENT_LIFECYCLE_COMPLETED)
    {
        SetEntityEventScript(ENTITY_CHILD, EventScript_FamilyEvent_Mary_SpouseBirthdayDinnerBabyGooGoo);
    }
