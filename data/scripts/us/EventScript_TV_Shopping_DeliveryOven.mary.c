...
mary_text_table
{
    const char gText_TV_Shopping_DeliveryOven_TheOvenYouOrderedIsHere[] =
        "The Oven you ordered \r\n"
        "is here. {Press}\p"
        "Can't bake anything without\r\n"
        "this baby!\r\n"
        "It's top quality, too!{Press}";

    const char gText_TV_Shopping_DeliveryOven_DeliveredSafeAndSound[] =
        "Delivered safe and sound!{Press}";
};

void EventScript_TV_Shopping_DeliveryOven(void)
{
    ...
    VarSet(VAR_TV_SHOPPING_OVEN_DELIVERY_STATE, TV_SHOPPING_DELIVERY_IN_PROGRESS);
    TalkOpen();
    SetTalkPortrait(TALK_PORTRAIT_ZACK_NORMAL);
    SetTalkNameplateCharacter(CHARACTER_ZACK);
    TalkMessage(gText_TV_Shopping_DeliveryOven_TheOvenYouOrderedIsHere);
    SetTalkPortrait(TALK_PORTRAIT_ZACK_HAPPY);
    SetTalkNameplateCharacter(CHARACTER_ZACK);
    TalkMessage(gText_TV_Shopping_DeliveryOven_DeliveredSafeAndSound);
    TalkClose();
    ...
    VarSet(VAR_HAS_KITCHEN_OVEN, TRUE);
    VarSet(VAR_TV_SHOPPING_OVEN_DELIVERY_STATE, TV_SHOPPING_DELIVERY_COMPLETED);
    return;
}
