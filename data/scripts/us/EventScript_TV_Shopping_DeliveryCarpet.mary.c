...
mary_text_table
{
    const char gText_TV_Shopping_DeliveryCarpet_TheCarpetYouOrderedIsHere[] =
        "The Carpet you ordered \r\n"
        "is here. {Press}\p"
        "This'll really cheer up a \r\n"
        "room!\r\n"
        "It's top quality, too!{Press}";

    const char gText_TV_Shopping_DeliveryCarpet_DeliveredSafeAndSound[] =
        "Delivered safe and sound!{Press}";
};

void EventScript_TV_Shopping_DeliveryCarpet(void)
{
    ...
    VarSet(VAR_TV_SHOPPING_CARPET_DELIVERY_STATE, TV_SHOPPING_DELIVERY_IN_PROGRESS);
    TalkOpen();
    SetTalkPortrait(TALK_PORTRAIT_ZACK_NORMAL);
    SetTalkNameplateCharacter(CHARACTER_ZACK);
    TalkMessage(gText_TV_Shopping_DeliveryCarpet_TheCarpetYouOrderedIsHere);
    SetTalkPortrait(TALK_PORTRAIT_ZACK_HAPPY);
    SetTalkNameplateCharacter(CHARACTER_ZACK);
    TalkMessage(gText_TV_Shopping_DeliveryCarpet_DeliveredSafeAndSound);
    TalkClose();
    ...
    VarSet(VAR_HAS_CARPET, TRUE);
    VarSet(VAR_TV_SHOPPING_CARPET_DELIVERY_STATE, TV_SHOPPING_DELIVERY_COMPLETED);
    return;
}
