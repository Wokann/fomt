...
mary_text_table
{
    const char gText_TV_Shopping_DeliveryMirror_TheMirrorYouOrderedIsHere[] =
        "The Mirror you ordered \r\n"
        "is here. {Press}\p"
        "A man's gotta take care of \r\n"
        "his looks!\r\n"
        "It's top quality, too!{Press}";

    const char gText_TV_Shopping_DeliveryMirror_DeliveredSafeAndSound[] =
        "Delivered safe and sound!{Press}";
};

void EventScript_TV_Shopping_DeliveryMirror(void)
{
    ...
    VarSet(VAR_TV_SHOPPING_MIRROR_DELIVERY_STATE, TV_SHOPPING_DELIVERY_IN_PROGRESS);
    TalkOpen();
    SetTalkPortrait(TALK_PORTRAIT_ZACK_NORMAL);
    SetTalkNameplateCharacter(CHARACTER_ZACK);
    TalkMessage(gText_TV_Shopping_DeliveryMirror_TheMirrorYouOrderedIsHere);
    SetTalkPortrait(TALK_PORTRAIT_ZACK_HAPPY);
    SetTalkNameplateCharacter(CHARACTER_ZACK);
    TalkMessage(gText_TV_Shopping_DeliveryMirror_DeliveredSafeAndSound);
    TalkClose();
    ...
    VarSet(VAR_HAS_MIRROR, TRUE);
    VarSet(VAR_TV_SHOPPING_MIRROR_DELIVERY_STATE, TV_SHOPPING_DELIVERY_COMPLETED);
    return;
}
