...
mary_text_table
{
    const char gText_TV_Shopping_DeliveryWhisk_TheWhiskYouOrderedIsHere[] =
        "The Whisk you ordered \r\n"
        "is here. {Press}\p"
        "Can't make Cakes without\r\n"
        "one of these.\r\n"
        "It's top quality, too!{Press}";

    const char gText_TV_Shopping_DeliveryWhisk_DeliveredSafeAndSound[] =
        "Delivered safe and sound!{Press}";
};

void EventScript_TV_Shopping_DeliveryWhisk(void)
{
    ...
    VarSet(VAR_TV_SHOPPING_WHISK_DELIVERY_STATE, TV_SHOPPING_DELIVERY_IN_PROGRESS);
    TalkOpen();
    SetTalkPortrait(TALK_PORTRAIT_ZACK_NORMAL);
    SetTalkNameplateCharacter(CHARACTER_ZACK);
    TalkMessage(gText_TV_Shopping_DeliveryWhisk_TheWhiskYouOrderedIsHere);
    SetTalkPortrait(TALK_PORTRAIT_ZACK_HAPPY);
    SetTalkNameplateCharacter(CHARACTER_ZACK);
    TalkMessage(gText_TV_Shopping_DeliveryWhisk_DeliveredSafeAndSound);
    TalkClose();
    ...
    VarSet(VAR_HAS_KITCHEN_WHISK, TRUE);
    VarSet(VAR_TV_SHOPPING_WHISK_DELIVERY_STATE, TV_SHOPPING_DELIVERY_COMPLETED);
    return;
}
