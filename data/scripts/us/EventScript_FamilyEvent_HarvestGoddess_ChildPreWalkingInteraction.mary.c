...
mary_text_table
{
    const char gText_FamilyEvent_HarvestGoddess_ChildPreWalkingInteraction_Response[] =
        "Goo. {Press}";
};

void EventScript_FamilyEvent_HarvestGoddess_ChildPreWalkingInteraction(void)
{
    if (VarGet(VAR_HARVEST_GODDESS_CHILDBIRTH_EVENT_STATE) == EVENT_LIFECYCLE_COMPLETED)
    {
        if (VarGet(VAR_CHILD_AGE_DAYS) >= CHILD_AGE_DAYS_FAMILY_SCENE_AND_INJURY_EVENT_START)
        {
            ...
            TalkMessage(gText_FamilyEvent_HarvestGoddess_ChildPreWalkingInteraction_Response);
            TalkClose();
