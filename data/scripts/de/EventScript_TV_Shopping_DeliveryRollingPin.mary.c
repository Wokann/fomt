...
mary_text_table
{
    const char gText_TV_Shopping_DeliveryRollingPin_TheRollingPinYouOrderedIs[] =
        "Das Nudelholz, das du \r\n"
        "bestellt hast, ist da. {Press}\p"
        "Ohne das Nudelholz kannst \r\n"
        "du keine Nudeln machen!\r\n"
        "Es ist auch Top-Qualität!{Press}";

    const char gText_TV_Shopping_DeliveryRollingPin_DeliveredSafeAndSound[] =
        "Unversehrt geliefert!{Press}";
};

void EventScript_TV_Shopping_DeliveryRollingPin(void)
{
    ...
    VarSet(VAR_TV_SHOPPING_ROLLING_PIN_DELIVERY_STATE, TV_SHOPPING_DELIVERY_IN_PROGRESS);
    TalkOpen();
    SetTalkPortrait(TALK_PORTRAIT_ZACK_NORMAL);
    SetTalkNameplateCharacter(CHARACTER_ZACK);
    TalkMessage(gText_TV_Shopping_DeliveryRollingPin_TheRollingPinYouOrderedIs);
    SetTalkPortrait(TALK_PORTRAIT_ZACK_HAPPY);
    SetTalkNameplateCharacter(CHARACTER_ZACK);
    TalkMessage(gText_TV_Shopping_DeliveryRollingPin_DeliveredSafeAndSound);
    TalkClose();
    ...
    VarSet(VAR_HAS_KITCHEN_ROLLING_PIN, TRUE);
    VarSet(VAR_TV_SHOPPING_ROLLING_PIN_DELIVERY_STATE, TV_SHOPPING_DELIVERY_COMPLETED);
    return;
}
