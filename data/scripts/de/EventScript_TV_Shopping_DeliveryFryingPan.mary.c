...
mary_text_table
{
    const char gText_TV_Shopping_DeliveryFryingPan_TheFryingPanYouOrderedIs[] =
        "Die Bratpfanne, die du\r\n"
        "bestellt hast, ist da. {Press}\p"
        "Ohne eine Bratpfanne \r\n"
        "kannst du nichts braten. \r\n"
        "Sie ist auch Top-Qualität!{Press}";

    const char gText_TV_Shopping_DeliveryFryingPan_DeliveredSafeAndSound[] =
        "Unversehrt geliefert!{Press}";
};

void EventScript_TV_Shopping_DeliveryFryingPan(void)
{
    ...
    VarSet(VAR_TV_SHOPPING_FRYING_PAN_DELIVERY_STATE, TV_SHOPPING_DELIVERY_IN_PROGRESS);
    TalkOpen();
    SetTalkPortrait(TALK_PORTRAIT_ZACK_NORMAL);
    SetTalkNameplateCharacter(CHARACTER_ZACK);
    TalkMessage(gText_TV_Shopping_DeliveryFryingPan_TheFryingPanYouOrderedIs);
    SetTalkPortrait(TALK_PORTRAIT_ZACK_HAPPY);
    SetTalkNameplateCharacter(CHARACTER_ZACK);
    TalkMessage(gText_TV_Shopping_DeliveryFryingPan_DeliveredSafeAndSound);
    TalkClose();
    ...
    VarSet(VAR_HAS_KITCHEN_FRYING_PAN, TRUE);
    VarSet(VAR_TV_SHOPPING_FRYING_PAN_DELIVERY_STATE, TV_SHOPPING_DELIVERY_COMPLETED);
    return;
}
