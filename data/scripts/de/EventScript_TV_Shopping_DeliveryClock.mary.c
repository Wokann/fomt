...
mary_text_table
{
    const char gText_TV_Shopping_DeliveryClock_HeyTheClockYouOrderedFinally[] =
        "He, die Uhr, die du bestellt\r\n"
        "hast, ist endlich da.{Press}\p"
        "Sie ist auch Top-Qualität!{Press}";

    const char gText_TV_Shopping_DeliveryClock_DeliveredSafeAndSound[] =
        "Unversehrt geliefert!{Press}";
};

void EventScript_TV_Shopping_DeliveryClock(void)
{
    ...
    VarSet(VAR_TV_SHOPPING_CLOCK_DELIVERY_STATE, TV_SHOPPING_DELIVERY_IN_PROGRESS);
    TalkOpen();
    SetTalkPortrait(TALK_PORTRAIT_ZACK_NORMAL);
    SetTalkNameplateCharacter(CHARACTER_ZACK);
    TalkMessage(gText_TV_Shopping_DeliveryClock_HeyTheClockYouOrderedFinally);
    SetTalkPortrait(TALK_PORTRAIT_ZACK_HAPPY);
    SetTalkNameplateCharacter(CHARACTER_ZACK);
    TalkMessage(gText_TV_Shopping_DeliveryClock_DeliveredSafeAndSound);
    TalkClose();
    ...
    VarSet(VAR_HAS_CLOCK, TRUE);
    VarSet(VAR_TV_SHOPPING_CLOCK_DELIVERY_STATE, TV_SHOPPING_DELIVERY_COMPLETED);
    return;
}
