...
mary_text_table
{
    const char gText_FamilyEvent_Popuri_PlayerBirthdayDinnerBabyGooGoo[] =
        "Ga-ga.{Press}";
};

void EventScript_FamilyEvent_Popuri_PlayerBirthdayDinnerBabyGooGoo(void)
{
    if (VarGet(VAR_POPURI_CHILDBIRTH_EVENT_STATE) == EVENT_LIFECYCLE_COMPLETED)
    {
        if (VarGet(VAR_CHILD_AGE_DAYS) >= CHILD_AGE_DAYS_FAMILY_SCENE_AND_INJURY_EVENT_START)
        {
            ...
            TalkMessage(gText_FamilyEvent_Popuri_PlayerBirthdayDinnerBabyGooGoo);
            TalkClose();
