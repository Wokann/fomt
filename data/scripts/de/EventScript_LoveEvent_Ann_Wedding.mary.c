...
mary_text_table
{
    const char gText_LoveEvent_Ann_Wedding_CarterPronouncesMarriage[] =
        "Du darfst die Braut küssen.{Press}";

    const char gText_LoveEvent_Ann_Wedding_AnnPromisesHerBest[] =
        "Ich tue mein Bestes. {Press}";

    const char gText_LoveEvent_Ann_Wedding_CarterBlessesCouple[] =
        "Mögt ihr für immer \r\n"
        "glücklich sein.{Press}";
};

void EventScript_LoveEvent_Ann_Wedding(void)
{
    ...
    TalkMessage(gText_LoveEvent_Ann_Wedding_CarterPronouncesMarriage);
    TalkClose();
    ...
    TalkMessage(gText_LoveEvent_Ann_Wedding_AnnPromisesHerBest);
    TalkClose();
    ...
    TalkMessage(gText_LoveEvent_Ann_Wedding_CarterBlessesCouple);
    TalkClose();
    SetEntityAnim(ENTITY_PLAYER, ANIMATION_PLAYER_WEDDING_WALK);
    ...
    MoveEntityYTo(ENTITY_ANN, Y(280), ENTITY_MOVE_SPEED_1_PIXEL_PER_FRAME);
    ...
    AddNpcFriendship(CHARACTER_DOUG, 255);
    ...
    VarSet(VAR_ANN_MARRIAGE_STATE, MARRIAGE_STATE_MARRIED);
    FadeOutScreen(SCREEN_FADE_STYLE_WHITE, SCREEN_FADE_SPEED_NORMAL);
    RunStaffCredits();
    CallScript(EventScript_LoveEvent_Ann_NicknameChoice);
}
