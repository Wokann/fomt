...
mary_text_table
{
    const char gText_FamilyEvent_Ann_PregnancyDiscovery_AnnFeelsUnwellAtFarmhouse[] =
        ...
        "so schlecht...{Press}\p"
        "{NickName}, kommst du \r\n"
        "mit mir in die Klinik?{Press}";

    const char gText_FamilyEvent_Ann_PregnancyDiscovery_Shared_AnnThanksPlayerAndDoctorPregnancyExplanation[] =
        "Danke.{Press}";
...
void EventScript_FamilyEvent_Ann_PregnancyDiscovery(void)
{
    ...
    ShowTalkHeartIndicator(CHARACTER_ANN);
    TalkMessage(gText_FamilyEvent_Ann_PregnancyDiscovery_Shared_AnnThanksPlayerAndDoctorPregnancyExplanation);
    TalkClose();
