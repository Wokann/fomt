...
mary_text_table
{
    const char gText_TV_Shopping_DeliveryPowerBerry_ThePowerBerryYouOrderedIs[] =
        "Die Power-Beere, die du\r\n"
        "bestellt hast, ist da. {Press}\p"
        "Sie wird deine Ausdauer\r\n"
        "wirklich verstärken!\r\n"
        "Sie ist auch Top-Qualität!{Press}";

    const char gText_TV_Shopping_DeliveryPowerBerry_DeliveredSafeAndSound[] =
        "Unversehrt geliefert!{Press}";
};

void EventScript_TV_Shopping_DeliveryPowerBerry(void)
{
    ...
    VarSet(VAR_TV_SHOPPING_POWER_BERRY_DELIVERY_STATE, TV_SHOPPING_DELIVERY_IN_PROGRESS);
    TalkOpen();
    SetTalkPortrait(TALK_PORTRAIT_ZACK_NORMAL);
    SetTalkNameplateCharacter(CHARACTER_ZACK);
    TalkMessage(gText_TV_Shopping_DeliveryPowerBerry_ThePowerBerryYouOrderedIs);
    SetTalkPortrait(TALK_PORTRAIT_ZACK_HAPPY);
    SetTalkNameplateCharacter(CHARACTER_ZACK);
    TalkMessage(gText_TV_Shopping_DeliveryPowerBerry_DeliveredSafeAndSound);
    TalkClose();
    ...
    VarSet(VAR_TV_SHOPPING_POWER_BERRY_DELIVERY_STATE, TV_SHOPPING_DELIVERY_COMPLETED);
    return;
}
