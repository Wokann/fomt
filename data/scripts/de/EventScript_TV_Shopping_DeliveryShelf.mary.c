...
mary_text_table
{
    const char gText_TV_Shopping_DeliveryShelf_TheShelfYouOrderedIsHere[] =
        "Das Regal, was du bestellt\r\n"
        "hast, ist da. {Press}\p"
        "Darin kannst du nun viele \r\n"
        "Dinge lagern. \r\n"
        "Er ist auch Top-Qualität!{Press}";

    const char gText_TV_Shopping_DeliveryShelf_DeliveredSafeAndSound[] =
        "Unversehrt geliefert!{Press}";
};

void EventScript_TV_Shopping_DeliveryShelf(void)
{
    ...
    VarSet(VAR_TV_SHOPPING_SHELF_DELIVERY_STATE, TV_SHOPPING_DELIVERY_IN_PROGRESS);
    TalkOpen();
    SetTalkPortrait(TALK_PORTRAIT_ZACK_NORMAL);
    SetTalkNameplateCharacter(CHARACTER_ZACK);
    TalkMessage(gText_TV_Shopping_DeliveryShelf_TheShelfYouOrderedIsHere);
    SetTalkPortrait(TALK_PORTRAIT_ZACK_HAPPY);
    SetTalkNameplateCharacter(CHARACTER_ZACK);
    TalkMessage(gText_TV_Shopping_DeliveryShelf_DeliveredSafeAndSound);
    TalkClose();
    ...
    VarSet(VAR_HAS_SHELF, TRUE);
    VarSet(VAR_TV_SHOPPING_SHELF_DELIVERY_STATE, TV_SHOPPING_DELIVERY_COMPLETED);
    return;
}
