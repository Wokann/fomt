...
mary_text_table
{
    const char gText_AchievementEvent_SpringMine_ReachedB200[] =
        "Ta-daaaa!\r\n"
        "Du hast B200 erreicht.{Press}\p"
        "Das ist alles.\r\n"
        "Es geht noch weiter.\r\n"
        "Viel Glück!  Tschüss.{Press}";
};

void EventScript_AchievementEvent_SpringMine_ReachedB200(void)
{
    VarSet(VAR_ACHIEVEMENT_REACHED_SPRING_MINE_B200_STATE, ACHIEVEMENT_EVENT_PRESENTING);
    ...
    TalkMessage(gText_AchievementEvent_SpringMine_ReachedB200);
    TalkClose();
    MarkNpcSpokenTo(CHARACTER_HARVEST_GODDESS);
    AddCharacterLove(CHARACTER_HARVEST_GODDESS, 3000);
    VarSet(VAR_ACHIEVEMENT_REACHED_SPRING_MINE_B200_STATE, ACHIEVEMENT_EVENT_COMPLETED);
