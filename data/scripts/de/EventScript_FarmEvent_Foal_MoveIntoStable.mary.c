...
mary_text_table
{
    const char gText_FarmEvent_Foal_MoveIntoStable_Barley_ReprimandsHorseName[] =
        "Hi, {Player}.\r\n"
        "Das nennst du Farm?\r\n"
        "Ohne ein einziges Pferd?{Press}";

    const char gText_FarmEvent_Foal_MoveIntoStable_Barley_ExplainsFoalNeedsHome[] =
        "Dieses Fohlen ist auf\r\n"
        "unserer Farm geboren, aber\r\n"
        "behalte es, wenn du willst.{Press}\p"
        "Du kannst es nennen,\r\n"
        "wie du möchtest.{Press}";

    const char gText_FarmEvent_Foal_MoveIntoStable_Barley_MovesFoalToStable[] =
        "Ich bringe es für \r\n"
        "dich in den Stall.{Press}\p"
        "Behandle ihn gut.\r\n"
        "Rede täglich mit ihm und\r\n"
        "bürste es auch. {Press}";
