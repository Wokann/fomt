...
mary_text_table
{
    const char gText_AchievementEvent_Pedometer_100000000Steps[] =
        "Ta-daaaa!{Press}\p"
        "Dein Schrittmesser hat\r\n"
        "gerade 100.000.000 gezählt.{Press}\p"
...
void EventScript_AchievementEvent_Pedometer_100000000Steps(void)
{
    VarSet(VAR_ACHIEVEMENT_WALKED_100000000_STEPS_STATE, ACHIEVEMENT_EVENT_PRESENTING);
    ...
    TalkMessage(gText_AchievementEvent_Pedometer_100000000Steps);
    TalkClose();
    MarkNpcSpokenTo(CHARACTER_HARVEST_GODDESS);
    AddCharacterLove(CHARACTER_HARVEST_GODDESS, 3000);
    VarSet(VAR_ACHIEVEMENT_WALKED_100000000_STEPS_STATE, ACHIEVEMENT_EVENT_COMPLETED);
