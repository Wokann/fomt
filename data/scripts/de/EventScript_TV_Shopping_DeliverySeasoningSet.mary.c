...
mary_text_table
{
    const char gText_TV_Shopping_DeliverySeasoningSet_TheSeasoningSetYouOrderedIs[] =
        "Das Gewürzset, das du\r\n"
        "du bestellt hast, ist da. {Press}\p"
        "Ohne Gewürze kannst du\r\n"
        "kein leckeres Essen machen.\r\n"
        "Sie sind auch Top-Qualität!{Press}";

    const char gText_TV_Shopping_DeliverySeasoningSet_DeliveredSafeAndSound[] =
        "Unversehrt geliefert!{Press}";
};

void EventScript_TV_Shopping_DeliverySeasoningSet(void)
{
    ...
    VarSet(VAR_TV_SHOPPING_SEASONING_SET_DELIVERY_STATE, TV_SHOPPING_DELIVERY_IN_PROGRESS);
    TalkOpen();
    SetTalkPortrait(TALK_PORTRAIT_ZACK_NORMAL);
    SetTalkNameplateCharacter(CHARACTER_ZACK);
    TalkMessage(gText_TV_Shopping_DeliverySeasoningSet_TheSeasoningSetYouOrderedIs);
    SetTalkPortrait(TALK_PORTRAIT_ZACK_HAPPY);
    SetTalkNameplateCharacter(CHARACTER_ZACK);
    TalkMessage(gText_TV_Shopping_DeliverySeasoningSet_DeliveredSafeAndSound);
    TalkClose();
    ...
    VarSet(VAR_HAS_KITCHEN_SEASONING_SET, TRUE);
    VarSet(VAR_TV_SHOPPING_SEASONING_SET_DELIVERY_STATE, TV_SHOPPING_DELIVERY_COMPLETED);
    return;
}
