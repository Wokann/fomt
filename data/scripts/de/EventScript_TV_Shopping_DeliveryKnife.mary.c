...
mary_text_table
{
    const char gText_TV_Shopping_DeliveryKnife_TheKnifeYouOrderedIsHere[] =
        "Das Messer, das du \r\n"
        "bestellt hast, ist da. {Press}\p"
        "Ohne so ein Messer kannst\r\n"
        "du nichts schneiden!\r\n"
        "Es ist auch Top-Qualität!{Press}";

    const char gText_TV_Shopping_DeliveryKnife_DeliveredSafeAndSound[] =
        "Unversehrt geliefert!{Press}";
};

void EventScript_TV_Shopping_DeliveryKnife(void)
{
    ...
    VarSet(VAR_TV_SHOPPING_KNIFE_DELIVERY_STATE, TV_SHOPPING_DELIVERY_IN_PROGRESS);
    TalkOpen();
    SetTalkPortrait(TALK_PORTRAIT_ZACK_NORMAL);
    SetTalkNameplateCharacter(CHARACTER_ZACK);
    TalkMessage(gText_TV_Shopping_DeliveryKnife_TheKnifeYouOrderedIsHere);
    SetTalkPortrait(TALK_PORTRAIT_ZACK_HAPPY);
    SetTalkNameplateCharacter(CHARACTER_ZACK);
    TalkMessage(gText_TV_Shopping_DeliveryKnife_DeliveredSafeAndSound);
    TalkClose();
    ...
    VarSet(VAR_HAS_KITCHEN_KNIFE, TRUE);
    VarSet(VAR_TV_SHOPPING_KNIFE_DELIVERY_STATE, TV_SHOPPING_DELIVERY_COMPLETED);
    return;
}
