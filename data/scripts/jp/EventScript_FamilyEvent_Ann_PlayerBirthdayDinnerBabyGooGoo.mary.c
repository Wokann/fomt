...
mary_text_table
{
    const char gText_FamilyEvent_Ann_PlayerBirthdayDinnerBabyGooGoo[] =
        "だあ？{Press}";
};

void EventScript_FamilyEvent_Ann_PlayerBirthdayDinnerBabyGooGoo(void)
{
    if (VarGet(VAR_ANN_CHILDBIRTH_EVENT_STATE) == EVENT_LIFECYCLE_COMPLETED)
    {
        if (VarGet(VAR_CHILD_AGE_DAYS) >= CHILD_AGE_DAYS_FAMILY_SCENE_AND_INJURY_EVENT_START)
        {
            ...
            TalkMessage(gText_FamilyEvent_Ann_PlayerBirthdayDinnerBabyGooGoo);
            TalkClose();
