...
mary_text_table
{
    ...
    const char gText_LocationInteraction_InspectBeachCottageClock_MorningTimeZeroPaddedMinutes[] =
        "午前{Var1}時０{Var2}分{Press}";

    const char gText_LocationInteraction_InspectBeachCottageClock_MorningTimeMinutes[] =
        "午前{Var1}時{Var2}分{Press}";

    const char gText_LocationInteraction_InspectBeachCottageClock_AfternoonTimeSpacedZeroPaddedMinutes[] =
        "午後　{Var1}時０{Var2}分{Press}";

    const char gText_LocationInteraction_InspectBeachCottageClock_AfternoonTimeSpacedMinutes[] =
        "午後　{Var1}時{Var2}分{Press}";

    const char gText_LocationInteraction_InspectBeachCottageClock_AfternoonTimeZeroPaddedMinutes[] =
        "午後{Var1}時０{Var2}分{Press}";

    const char gText_LocationInteraction_InspectBeachCottageClock_AfternoonTimeMinutes[] =
        "午後{Var1}時{Var2}分{Press}";
};
...
void EventScript_LocationInteraction_InspectBeachCottageClock(void)
{
    ...
    if (VarGet(VAR_HOUR) < 12)
    {
        ...
        else
        {
            if (VarGet(VAR_MINUTE) < 10)
            {
                TalkAppendMessage(gText_LocationInteraction_InspectBeachCottageClock_MorningTimeZeroPaddedMinutes);
            }
            else
            {
                TalkAppendMessage(gText_LocationInteraction_InspectBeachCottageClock_MorningTimeMinutes);
            }
        }
    }
    else
    {
        ...
        if (VarGet(VAR_HOUR) < 22)
        {
            if (VarGet(VAR_MINUTE) < 10)
            {
                TalkAppendMessage(gText_LocationInteraction_InspectBeachCottageClock_AfternoonTimeSpacedZeroPaddedMinutes);
            }
            else
            {
                TalkAppendMessage(gText_LocationInteraction_InspectBeachCottageClock_AfternoonTimeSpacedMinutes);
            }
        }
        else
        {
            if (VarGet(VAR_MINUTE) < 10)
            {
                TalkAppendMessage(gText_LocationInteraction_InspectBeachCottageClock_AfternoonTimeZeroPaddedMinutes);
            }
            else
            {
                TalkAppendMessage(gText_LocationInteraction_InspectBeachCottageClock_AfternoonTimeMinutes);
            }
        }
    }
    TalkClose();
