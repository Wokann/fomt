...
mary_text_table
{
    const char gText_FamilyEvent_Karen_SpouseBirthdayDinnerBabyBabbles[] =
        "だあ？{Press}";
};

void EventScript_FamilyEvent_Karen_SpouseBirthdayDinnerBabyBabbles(void)
{
    if (VarGet(VAR_KAREN_CHILDBIRTH_EVENT_STATE) == EVENT_LIFECYCLE_COMPLETED)
    {
        if (VarGet(VAR_CHILD_AGE_DAYS) >= CHILD_AGE_DAYS_FAMILY_SCENE_AND_INJURY_EVENT_START)
        {
            ...
            TalkMessage(gText_FamilyEvent_Karen_SpouseBirthdayDinnerBabyBabbles);
            TalkClose();
