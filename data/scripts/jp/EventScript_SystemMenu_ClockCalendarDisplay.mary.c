...
mary_text_table
{
    ...
    const char gText_SystemMenu_ClockCalendarDisplay_MorningTimeZeroPaddedMinutes[] =
        "午前{Var1}時０{Var2}分{Press}";

    const char gText_SystemMenu_ClockCalendarDisplay_MorningTimeMinutes[] =
        "午前{Var1}時{Var2}分{Press}";

    const char gText_SystemMenu_ClockCalendarDisplay_AfternoonTimeSpacedZeroPaddedMinutes[] =
        "午後　{Var1}時０{Var2}分{Press}";

    const char gText_SystemMenu_ClockCalendarDisplay_AfternoonTimeSpacedMinutes[] =
...
void EventScript_SystemMenu_ClockCalendarDisplay(void)
{
    ...
    if (VarGet(VAR_HOUR) < 12)
    {
        ...
        else
        {
            if (VarGet(VAR_MINUTE) < 10)
            {
                TalkAppendMessage(gText_SystemMenu_ClockCalendarDisplay_MorningTimeZeroPaddedMinutes);
            }
            else
            {
                TalkAppendMessage(gText_SystemMenu_ClockCalendarDisplay_MorningTimeMinutes);
            }
        }
    }
    else
    {
        ...
        if (VarGet(VAR_HOUR) < 22)
        {
            if (VarGet(VAR_MINUTE) < 10)
            {
                TalkAppendMessage(gText_SystemMenu_ClockCalendarDisplay_AfternoonTimeSpacedZeroPaddedMinutes);
            }
            else
            {
                TalkAppendMessage(gText_SystemMenu_ClockCalendarDisplay_AfternoonTimeSpacedMinutes);
            }
        }
        else
        {
