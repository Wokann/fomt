...
mary_text_table
{
    const char gText_AchievementEvent_Shipping_10000Items[] =
        "Ta-daaaa!{Press}\p"
        "You have shipped 10,000\r\n"
        "{Var1}.{Press}\p"
...
void EventScript_AchievementEvent_Shipping_10000Items(void)
{
    VarSet(VAR_ACHIEVEMENT_SHIPPED_10000_ITEMS_STATE, ACHIEVEMENT_EVENT_PRESENTING);
    ...
    TalkMessage(gText_AchievementEvent_Shipping_10000Items);
    TalkClose();
    MarkNpcSpokenTo(CHARACTER_HARVEST_GODDESS);
    AddCharacterLove(CHARACTER_HARVEST_GODDESS, 3000);
    VarSet(VAR_ACHIEVEMENT_SHIPPED_10000_ITEMS_STATE, ACHIEVEMENT_EVENT_COMPLETED);
