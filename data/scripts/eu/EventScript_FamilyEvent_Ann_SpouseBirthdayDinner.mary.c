...
void EventScript_FamilyEvent_Ann_SpouseBirthdayDinner(void)
{
    ...
    SetEntityPosition(ENTITY_ANN, X(327), Y(114), FACING_DOWN);
    ...
    CreateEventIcon(EVENT_ICON_SLOT_1, X(304), Y(124), EVENT_ICON_LAYER_LOW_PRIORITY, GetFoodIconId(ITEM_FOOD_GRAPE_JUICE));
    FadeInScreen(SCREEN_FADE_STYLE_BLACK_MOSAIC, SCREEN_FADE_SPEED_NORMAL);
    EnableScriptedNpcControl();
    SetEntityEventScript(ENTITY_ANN, EventScript_FamilyEvent_Ann_SpouseBirthdayDinnerDialogueChoice);
    if (VarGet(VAR_ANN_CHILDBIRTH_EVENT_STATE) == EVENT_LIFECYCLE_COMPLETED)
    {
        SetEntityEventScript(ENTITY_CHILD, EventScript_FamilyEvent_Ann_SpouseBirthdayDinnerBabyGooGoo);
    }
