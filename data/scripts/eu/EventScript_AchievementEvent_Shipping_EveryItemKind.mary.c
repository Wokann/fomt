...
mary_text_table
{
    const char gText_AchievementEvent_Shipping_EveryItemKind[] =
        "Ta-daaaa!{Press}\p"
        "You have shipped every kind\r\n"
        "of items you can ship.{Press}\p"
...
void EventScript_AchievementEvent_Shipping_EveryItemKind(void)
{
    VarSet(VAR_ACHIEVEMENT_SHIPPED_EVERY_ITEM_KIND_STATE, ACHIEVEMENT_EVENT_PRESENTING);
    ...
    TalkMessage(gText_AchievementEvent_Shipping_EveryItemKind);
    TalkClose();
    MarkNpcSpokenTo(CHARACTER_HARVEST_GODDESS);
    AddCharacterLove(CHARACTER_HARVEST_GODDESS, 3000);
    VarSet(VAR_ACHIEVEMENT_SHIPPED_EVERY_ITEM_KIND_STATE, ACHIEVEMENT_EVENT_COMPLETED);
