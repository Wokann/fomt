...
mary_text_table
{
    const char gText_TV_Shopping_DeliveryPot_ThePotYouOrderedIsHere[] =
        "The Pot you ordered \r\n"
        "is here. {Press}\p"
        "Can't boil water without \r\n"
        "one of these!\r\n"
        "It's top quality, too!{Press}";

    const char gText_TV_Shopping_DeliveryPot_DeliveredSafeAndSound[] =
        "Delivered safe and sound!{Press}";
};

void EventScript_TV_Shopping_DeliveryPot(void)
{
    ...
    VarSet(VAR_TV_SHOPPING_POT_DELIVERY_STATE, TV_SHOPPING_DELIVERY_IN_PROGRESS);
    TalkOpen();
    SetTalkPortrait(TALK_PORTRAIT_ZACK_NORMAL);
    SetTalkNameplateCharacter(CHARACTER_ZACK);
    TalkMessage(gText_TV_Shopping_DeliveryPot_ThePotYouOrderedIsHere);
    SetTalkPortrait(TALK_PORTRAIT_ZACK_HAPPY);
    SetTalkNameplateCharacter(CHARACTER_ZACK);
    TalkMessage(gText_TV_Shopping_DeliveryPot_DeliveredSafeAndSound);
    TalkClose();
    ...
    VarSet(VAR_HAS_KITCHEN_POT, TRUE);
    VarSet(VAR_TV_SHOPPING_POT_DELIVERY_STATE, TV_SHOPPING_DELIVERY_COMPLETED);
    return;
}
