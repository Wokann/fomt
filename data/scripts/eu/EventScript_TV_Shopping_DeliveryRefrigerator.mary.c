...
mary_text_table
{
    const char gText_TV_Shopping_DeliveryRefrigerator_TheRefrigeratorYouOrderedIsHere[] =
        "The Refrigerator you ordered\r\n"
        "is here. {Press}\p"
        "Without a Refrigerator, you\r\n"
        "sure can't keep food fresh.\r\n"
        "It's top quality, too!{Press}";

    const char gText_TV_Shopping_DeliveryRefrigerator_DeliveredSafeAndSound[] =
        "Delivered safe and sound!{Press}";
};

void EventScript_TV_Shopping_DeliveryRefrigerator(void)
{
    ...
    VarSet(VAR_TV_SHOPPING_REFRIGERATOR_DELIVERY_STATE, TV_SHOPPING_DELIVERY_IN_PROGRESS);
    TalkOpen();
    SetTalkPortrait(TALK_PORTRAIT_ZACK_NORMAL);
    SetTalkNameplateCharacter(CHARACTER_ZACK);
    TalkMessage(gText_TV_Shopping_DeliveryRefrigerator_TheRefrigeratorYouOrderedIsHere);
    SetTalkPortrait(TALK_PORTRAIT_ZACK_HAPPY);
    SetTalkNameplateCharacter(CHARACTER_ZACK);
    TalkMessage(gText_TV_Shopping_DeliveryRefrigerator_DeliveredSafeAndSound);
    TalkClose();
    ...
    VarSet(VAR_HAS_REFRIGERATOR, TRUE);
    VarSet(VAR_TV_SHOPPING_REFRIGERATOR_DELIVERY_STATE, TV_SHOPPING_DELIVERY_COMPLETED);
    return;
}
