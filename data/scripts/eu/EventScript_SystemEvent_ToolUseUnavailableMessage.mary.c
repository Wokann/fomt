...
mary_text_table
{
    const char gText_SystemEvent_ToolUseUnavailableMessage_YouCantUseThatNow[] =
        "You can't use that now.{Press}";

    const char gText_SystemEvent_ToolUseUnavailableMessage_YouCantGoOutsideToday[] =
        "You can't go outside today.{Press}";
};

void EventScript_SystemEvent_ToolUseUnavailableMessage(void)
{
    ...
    {
        TalkOpen();
        TalkMessage(gText_SystemEvent_ToolUseUnavailableMessage_YouCantGoOutsideToday);
        TalkClose();
        return;
    }
    FadeOutScreen(SCREEN_FADE_STYLE_WHITE_MOSAIC, SCREEN_FADE_SPEED_NORMAL);
    OpenRucksackMenu(1);
    if (GetEntityLocation(ENTITY_PLAYER) != var_6)
    {
        switch (GetEntityLocation(ENTITY_PLAYER))
        {
            ...
            case MAP_MOTHERS_HILL_SUMMIT:
                if (VarGet(VAR_MOON_VIEWING_FESTIVAL_EVENT_STATE) == EVENT_LIFECYCLE_NOT_STARTED && var_0 == SEASON_FALL && var_1 == DAY_OF_MONTH_13)
                {
                    if (var_2 >= 18)
                    {
                        ...
                        {
                            if ((VarGet(VAR_RICK_KAREN_WEDDING_EVENT_STATE) == EVENT_LIFECYCLE_NOT_STARTED && VarGet(VAR_RICK_KAREN_WEDDING_POPURI_INTRO_EVENT_STATE) == EVENT_LIFECYCLE_NOT_STARTED && VarGet(VAR_RICK_KAREN_RIVAL_MARRIAGE_STATE) == MARRIAGE_STATE_SINGLE || VarGet(VAR_KAREN_MARRIAGE_STATE) == MARRIAGE_STATE_MARRIED) && GetCharacterLove(CHARACTER_KAREN) >= LOVE_HEART_GREEN_MIN || (VarGet(VAR_POPURI_KAI_WEDDING_EVENT_STATE) == EVENT_LIFECYCLE_NOT_STARTED && VarGet(VAR_POPURI_KAI_RIVAL_MARRIAGE_STATE) == MARRIAGE_STATE_SINGLE || VarGet(VAR_POPURI_MARRIAGE_STATE) == MARRIAGE_STATE_MARRIED) && GetCharacterLove(CHARACTER_POPURI) >= LOVE_HEART_GREEN_MIN || (VarGet(VAR_ANN_CLIFF_WEDDING_EVENT_STATE) == EVENT_LIFECYCLE_NOT_STARTED && VarGet(VAR_ANN_CLIFF_RIVAL_MARRIAGE_STATE) == MARRIAGE_STATE_SINGLE || VarGet(VAR_ANN_MARRIAGE_STATE) == MARRIAGE_STATE_MARRIED) && GetCharacterLove(CHARACTER_ANN) >= LOVE_HEART_GREEN_MIN || ((VarGet(VAR_MARY_GRAY_WEDDING_EVENT_STATE) == EVENT_LIFECYCLE_NOT_STARTED && VarGet(VAR_MARY_GRAY_RIVAL_MARRIAGE_STATE) == MARRIAGE_STATE_SINGLE || VarGet(VAR_MARY_MARRIAGE_STATE) == MARRIAGE_STATE_MARRIED) && GetCharacterLove(CHARACTER_MARY) >= LOVE_HEART_GREEN_MIN || (VarGet(VAR_ELLI_DOCTOR_WEDDING_EVENT_STATE) == EVENT_LIFECYCLE_NOT_STARTED && VarGet(VAR_ELLI_DOCTOR_RIVAL_MARRIAGE_STATE) == MARRIAGE_STATE_SINGLE || VarGet(VAR_ELLI_MARRIAGE_STATE) == MARRIAGE_STATE_MARRIED) && GetCharacterLove(CHARACTER_ELLI) >= LOVE_HEART_GREEN_MIN))
                            {
