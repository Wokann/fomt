#!/usr/bin/env python3
"""Prove one edited winter farm-status pixel fits the overseas native slot."""
...
STREAM_OFFSET_US = 0x52AA6C
STREAM_LENGTH = 0x1FD4
STREAM_SHA256 = "cfb5e29e03d3defef982b8132b1b3acd90b6798a40cfbeb92d0ac723ccef4812"
...
def main() -> None:
    ...
    if hashlib.sha256(baseline).hexdigest() != STREAM_SHA256:
        raise ValueError("US winter farm-status stream hash does not match the verified retail payload")
    ...
    edited[source_index] = (edited[source_index] + 1) & 0x0F
    ...
    print(
        f"winter farm-status edit test: pixel {source_index} changed; "
