...
旧 `agbcp` 还会偶尔在任何代码之前生成一个未被引用的匿名 `.LC` 文本。若它与
源码中第一个已定义的普通 `char const[]` 对象逐字节一致，预处理器会移除这个无消费者
副本，让命名对象自然占据该位置；只要匿名标签在汇编其他位置仍被引用，或声明顺序和
字节不完全吻合，就保持原样。该规则只依据声明顺序、字节和引用关系，不依赖特定文本、
变量名、函数或 ROM 地址。
