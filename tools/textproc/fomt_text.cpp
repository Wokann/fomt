...
// lowered fields must live at a chosen physical point inside an ordinary .cc
...
bool TryCompileStaffCreditsMarker(const std::string &source, const Charmap &charmap,
    const StaffCreditsInlineInput *staff_credits, std::size_t &at,
    std::ostringstream &output)
{
    constexpr std::string_view kMarker = "FOMT_STAFF_CREDITS";
    if (!StartsCppIdentifier(source, at, kMarker))
        return false;

    std::size_t cursor = at + kMarker.size();
    ...
    if (cursor == source.size() || source[cursor] != ')') {
        throw std::runtime_error("line " + std::to_string(SourceLineNumber(source, at))
            + ": FOMT_STAFF_CREDITS takes no arguments");
    }
    ++cursor;
    while (cursor < source.size()
        && std::isspace(static_cast<unsigned char>(source[cursor])) != 0) {
    ...
    if (cursor == source.size() || source[cursor] != ';') {
        throw std::runtime_error("line " + std::to_string(SourceLineNumber(source, at))
            + ": FOMT_STAFF_CREDITS must end with ';'");
    }
...
std::string CompileCppSourceText(const std::string &source, const Charmap &charmap,
    const StaffCreditsInlineInput *staff_credits = nullptr)
{
...
std::string GuideLineLabel(const std::string &page, std::size_t index,
    const std::string &text, bool has_fixed_title = true)
{
    // TITLE reserves slot zero for the fixed header. CONTINUATION instead
    // keeps that first physical pointer as Line00.
    if (index == 0 && has_fixed_title)
...
GuidePageSource ParseGuidePageSource(const std::string &source, const std::string &page)
{
    ...
    while (std::getline(lines, line)) {
        ...
        if (state == ParseState::TitleKeyword) {
            if (trimmed == "TITLE")
                result.has_fixed_title = true;
            else if (trimmed == "CONTINUATION")
                result.has_fixed_title = false;
            else {
                throw std::runtime_error("line " + std::to_string(line_number)
                    + ": expected TITLE or CONTINUATION at the start of the guide page");
            }
    ...
    const Bytes &rom, const Charmap &charmap, bool standalone_output = true)
{
    ...
    if (standalone_output) {
        // The standalone command writes a source file that will be passed
        // through CPP later, so its informational comment is safe there.
        output << "// Generated by fomt-text.  Do not edit.\n"
    ...
    if (IsGuidePageSource(source)) {
        ...
        for (std::size_t index = 0; index < page.lines.size(); ++index) {
            AddGuideCatalogText(catalog, charmap, page.lines[index],
                GuideLineLabel(page.page, index, page.lines[index], page.has_fixed_title));
        }
        return;
    }
...
void SelfTest()
{
    ...
    Require(GuideLineLabel("example", 0, "A", false)
            == "gText_ReferenceGuide_example_Line00",
        "guide continuation did not retain the first physical line slot");
    ...
    const GuidePageSource continuation_page = ParseGuidePageSource(
        "CONTINUATION\n"
        "\"A\"\n"
        "MAIN\n"
        "\"B\"\n"
